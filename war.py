...
def deal(Event=None):
    ...
    if len(cards) != 0:
        ...
        if botnum > playnum and stop == 0:  
            stop = 1 
            lose = Tk()
            wol = Label(lose, text="YOU LOSE!", font=("Arial", 75))
            wol.grid(row=0, column=0)
            restart = Button(lose, text="Restart?", font=("Arial", 25), command=lambda: start())
...
def windows():
    ...
    botnumbut = Button(window, text=botnum, font=("Arial", 25))
    botnumbut.grid(row=0, column=0)
    playnumbut = Button(window, text=playnum, font=("Arial", 25))
    playnumbut.grid(row=1, column=0)
    startbut = Button(window, text=playnum, font=("Arial", 25), command=lambda: deal())
